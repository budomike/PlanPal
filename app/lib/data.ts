...
export async function fetchFilteredEvents(
  ...
  try {
    const events = await sql<EventsTable>`
      ...
      events.description ILIKE ${`%${query}%`}) AND
      (events.date > ${currentDate} OR (events.date = ${currentDate} AND events.time >= ${currentDate.split('T')[1]}))
...
export async function fetchFilteredPastEvents(
  ...
  const currentDate = new Date().toISOString();
  try {
    const events = await sql<PastEventsTable>`
      SELECT
        events.id AS event_id,
        events.title,
        events.description,
